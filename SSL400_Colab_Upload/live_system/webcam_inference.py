...
Usage:
    python src/live_system/webcam_inference.py --exp_id 4
    python src/live_system/webcam_inference.py --exp_id 4 --camera 0
"""
...
def run_live_detection(
    ...
    # Initialize YOLOv8 for dynamic tracking
    try:
        from ultralytics import YOLO
        yolo_model = YOLO("yolov8n.pt")
        logger.info("YOLOv8 initialized for dynamic live cropping.")
    except ImportError:
        logger.warning("ultralytics not installed. YOLO tracking disabled.")
        yolo_model = None
        ...
        if yolo_model is not None:
            try:
                ...
                if len(results) > 0 and len(results[0].boxes) > 0:
                    ...
                    # Draw green bounding box on display
                    cv2.rectangle(display, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0), 2)
                    cv2.putText(display, "Tracking", (int(x1), max(10, int(y1)-10)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)

                    # Dynamic padding for crop
                    box_w, box_h = x2 - x1, y2 - y1
                    pad_w, pad_h = box_w * 0.15, box_h * 0.15
                    h, w, _ = frame.shape
                    
                    x_start, y_start = max(0, int(x1 - pad_w)), max(0, int(y1 - pad_h))
                    x_end, y_end = min(w, int(x2 + pad_w)), min(h, int(y2 + pad_h))
                    
                    cropped_attempt = frame[y_start:y_end, x_start:x_end]
                    if cropped_attempt.size > 0:
                        cropped_frame = cropped_attempt
            except Exception:
                pass
        else:
            ...
            y_end = int(h * 0.90)
            x_margin = int(w * 0.05)
            cropped_attempt = frame[0:y_end, x_margin:w-x_margin]
            if cropped_attempt.size > 0:
                cropped_frame = cropped_attempt
...
def main() -> None:
    ...
    parser.add_argument("--exp_id", type=int, default=4, choices=[1, 2, 3, 4],
